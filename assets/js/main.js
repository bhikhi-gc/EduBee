	...
	$.Sena_Theme = {
		...
		navigation:function() {
			...
			$('a[href^=\\#]').on("click", function(e) {
				if ($(this).attr('href')!=='#' && !$(e.target).parent().parent().is('.navbar-nav') && !$(this).attr('data-toggle')) {
					$(document).scrollTo($(this).attr('href'), Sena_Options.scrollSpeed, {offset:{top:-85, left:0}});
					e.preventDefault();
				}
			});
		...
		portfolio:function() {
			...
			$('a[href^="#view-"]').on("click", function() {
				...
				if ($item.length>0) {
					$(document).scrollTo('#portfolio', Sena_Options.scrollSpeed, {offset:{top:-85, left:0}, onAfter:function() {
						$item.trigger('click');
					}});
				}
			});
...
function shareTo(network, title, image, url) {
	return $.Sena_Theme.share(network, title, image, url);
}
